...
//Add your home network & API Key generated from coinmarketcap
const char* ssid     = "XXXXX";       ///EDIIIT
const char* password = "XXXXX"; //edit
...
void getData()
   {
    ...
    if ((WiFi.status() == WL_CONNECTED)) { //Check the current connection status
      HTTPClient http;
      http.begin("https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?symbol=BTC");
    ...
    if (httpCode > 0) { //Check for the returning code
      ...
      //Fetching the price in USD
      int startIndex = payload.indexOf("\"USD\":{\"price\":") + 16;
      int endIndex = payload.indexOf(",", startIndex);
      String priceStr = payload.substring(startIndex, endIndex);
      double price = priceStr.toFloat();

      //Aplly the price to the string
      String v=String(price);
